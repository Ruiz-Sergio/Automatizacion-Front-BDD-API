...
class Functions(Inicializar):
    ...
    def abrir_navegador(self, URL=Inicializar.URL, navegador=Inicializar.NAVEGADOR):
        ...
        if navegador == ("CHROME"):
            ...
            print(Inicializar.basedir + "\\drivers\\chromedriver.exe")
...
##########################################################################
 ...
 ##########################################################################
    ...
    # Se obtiene una lista(array) con todos los registros devueltos por la consulta.
    def get_list_base_mysql(self, host="", port="", user="", password="", database="", query=""):
        try:
            registros = []
            ...
            self.cursor = None
            pytest.skip("Error en la conexión de la Base de Datos.")
    ...
    def send_Service(tipoPeticion, endPoint, headers=None, payload=None, time=Inicializar.tiempo_entre_reintentos):
        ...
        '''
            ...
            Returns:
                Retorna un request si la petición es exitosa.
                De lo contrario imprime el error por consola.
        '''

        total_retry = Inicializar.cantidad_reintentos
        for retry in range(total_retry):
            if headers != None:
                response = requests.request(tipoPeticion, endPoint, headers=headers, data=payload, timeout=(time))
            else:
                response = requests.request(tipoPeticion, endPoint, timeout=(time))
